...
def load_and_run_detector(model_file, image_file_names, output_dir = None,
    ...
    def input_file_to_detection_file(fn, output_dir, crop_index=-1):
        """
        ...
        # Visualizing results on the image C:\Users\bkara\Downloads\temp\fox.jpg failed. Exception: '0000_fox_crop00_detections.jpg'
        ...
        try:
            ...
            for detection in result['detections']:
                if detection['category'] == '1':
                    ...
                    if confidence >= render_confidence_threshold:
                        print("True")
                    else:
                        print("False")
            ...
            continue
        max_confidence = max((d['conf'] for d in result['detections']), default=0)
        final_output_dir = true_output_dir if max_confidence >= render_confidence_threshold else false_output_dir
        ...
        try:
            if crop_images:
                ...
                for i_crop, cropped_image in enumerate(images_cropped):
                    output_full_path = input_file_to_detection_file(im_file, final_output_dir, i_crop)
                ...
                output_full_path = input_file_to_detection_file(im_file, final_output_dir)
                image.save(output_full_path)

        except Exception as e:
            print('Visualizing results on the image {} finalized. Exception: {}'.format(im_file, e))
            continue
...
def main():
    ...
    #true_output_dir = os.path.join(output_dir, 'true')
    #false_output_dir = os.path.join(output_dir, 'false')
    #os.makedirs(true_output_dir, exist_ok=True)
    #os.makedirs(false_output_dir, exist_ok=True)
...
#    main()
   ...
   # image_file_names.append("C:\\Users\\bkara\\Downloads\\temp\\Large.jpg")
